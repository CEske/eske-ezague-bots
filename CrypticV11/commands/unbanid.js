    ...
    con.query(`SELECT * FROM staffs WHERE staffid = '${userid}'`, (err1, row1) => {
      if(row1[0] && row1[0].rangering === 'Staff' || row1[0] && row1[0].rangering === 'Ledelse' || row1[0] && row1[0].rangering === 'bypass') {
          const whitelistEmbed = {
              color: 0x37ae5f,
              author: {
                  name: message.author.username + '#' + message.author.discriminator + ' har lige unbanned ID '+spillerid, 
                  icon_url: message.author.avatarURL
              },
              description: '**ID:** '+spillerid+' er nu unbanned'
          };
          fivem.query(`SELECT * FROM vrp_users WHERE id = '${spillerid}'`, (err2, row2) => {
              if (row2[0]) {
                  fivem.query(`UPDATE vrp_users SET banned = 0 WHERE id = '${spillerid}'`)
              }
          })
          message.delete()
